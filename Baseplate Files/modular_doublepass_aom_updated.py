...
def doublepass(x=0, y=0, angle=0, mirror=optomech.mirror_mount_m05, x_split=False, thumbscrews=True):
    ...
    beam = baseplate.add_beam_path(x=fb_newx, y=fb_newy-4, angle=layout.cardinal['right'], color = (0,0,255))

    baseplate.place_element("fp_in", optomech.fiberport_mount_hca3, x=fb_newx, 
                            y=fb_newy-4, angle=layout.cardinal['right'])
    ...
    baseplate.place_element_along_beam("mIn3", optomech.circular_mirror, beam, 
                                       beam_index=0b1, distance=mIn2_xpos-mIn1_xpos+fb_xpos-25, 
                                       angle=-135, mount_type=mirror, mount_args=dict(thumbscrews=thumbscrews))
    ...
    baseplate.place_element("mBack", optomech.circular_mirror, x=mOut1_xpos, 
                            y=mOut1_ypos, angle=-45, mount_type=mirror, mount_args=dict(thumbscrews=thumbscrews))
    ...
    baseplate.place_element("mOut2", optomech.circular_mirror, x=mOut2_xpos+66, 
                            y=mOut1_ypos, angle=225, mount_type=mirror, mount_args=dict(thumbscrews=thumbscrews))
    baseplate.place_element("mOut2", optomech.circular_mirror, x=mOut2_xpos+66, 
                            y=fb_newy-10, angle=45, mount_type=mirror, mount_args=dict(thumbscrews=thumbscrews))
    baseplate.place_element("fiberOut", optomech.fiberport_mount_hca3, x=base_dx, 
                            y=fb_newy-10, angle=layout.cardinal['left'])
    baseplate.place_element("mod_mountR", optomech.modular1, x=base_dx, 
                            y=fb_newy-10, angle=0)
